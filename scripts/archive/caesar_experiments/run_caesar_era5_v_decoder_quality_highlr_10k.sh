...
OUTPUT_DIR="${CAESAR_V_DECODER_HIGHLR_DIR:-$ROOT/checkpoints/caesar_era5_v_decoder_quality_highlr_10k}"
LOG_DIR="${CAESAR_V_DECODER_HIGHLR_LOG_DIR:-$ROOT/logs/caesar_era5_v_decoder_quality_highlr_10k}"
...

{
  ...
  printf 'data_dir=%s\n' "$DATA_DIR"
  printf 'source=%s\n' "$SOURCE"
  printf 'objective=frozen_encoder_decoder_quality_high_lr\n'
  sha256sum "$SOURCE"
...
common=(
  ...
  --val_interval 1000
  --save_interval 10000
  --milestone_steps 250 500 1000 2000 5000 10000
  --norm_type mean_range
  --ckpt_path "$SOURCE"
  --wandb_project caesar-era5-hourly-tuning
  --wandb_group v-lowrate-frozen-encoder-quality-highlr-10k
  --wandb_tags era5 low-rate decoder-only frozen-encoder quality-recovery high-lr
  --require_wandb
  --device cuda:0
)
...
launch 2 decoder 3e-5
launch 3 decoder 1e-4
launch 4 decoder 3e-4
launch 5 sr 3e-5
launch 6 sr 1e-4
launch 7 sr 3e-4
