...
SOURCE = {
    "caesar_v": ROOT / "unified_results_archive_20260611_035153/s2c_caesar_mid_eb_probe_v/summary.json",
    "caesar_d": ROOT / "unified_results_archive_20260611_035153/s2c_caesar_mid_eb_probe_d/summary.json",
}
...
def main() -> None:
    ...
    for model_id, path in SOURCE.items():
        points = [
            r
            for r in load(path)
        ...
        if len(points) != len(EBS):
            raise RuntimeError(f"{model_id}: expected {len(EBS)} points, got {len(points)}")
        ...
        for template, point in zip(templates, points):
            ...
            row["caesar_eb_source_summary"] = str(path.relative_to(ROOT))
        ...
        schedule[model_id] = {
            ...
            "note": "S2C old CAESAR eb=1..100 points saturated and collapsed to one plotted point; replaced with lower EB sweep.",
        }
    ...
    schedule_path = ROOT / "unified_results/s2c_caesar_effective_eb_schedules.json"
