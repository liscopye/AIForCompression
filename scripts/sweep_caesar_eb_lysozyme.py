...
def run_sweep(model_type: str, gpu: int, test_eb: float | None = None,
              output_root: str = "/workspace/AIForCompression/unified_results/lysozyme_caesar_tuned"):
    is_v = model_type == "V"
    nf = 8 if is_v else 16
    test_data = f"/workspace/Data/lysozyme_processed/lysozyme_test_nf{nf}.npz"

    output_base = os.path.join(output_root, model_type.lower())
    tuned_checkpoint = (
        "/workspace/AIForCompression/checkpoints/caesar_lysozyme/"
        f"caesar_{model_type.lower()}_tuning_lysozyme.pt"
    )
...
def main():
    ...
    p.add_argument(
        ...
        help="Root containing the v/ and d/ result directories.",
    )
    args = p.parse_args()

    if args.model_type == "both":
        for mt in ["V", "D"]:
            run_sweep(mt, args.gpu, args.test_eb, args.output_root)
    else:
        run_sweep(args.model_type, args.gpu, args.test_eb, args.output_root)
