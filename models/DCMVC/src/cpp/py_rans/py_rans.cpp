...
PYBIND11_MODULE(MLCodec_rans, m) {
  ...
  auto encoder = py::class_<RansEncoder>(m, "DCMVCRansEncoder", py::module_local())
  ...
  m.attr("RansEncoder") = encoder;

  auto decoder = py::class_<RansDecoder>(m, "DCMVCRansDecoder", py::module_local())
      .def(py::init<int>())
      .def("set_stream", &RansDecoder::set_stream)
      .def("decode_stream", &RansDecoder::decode_stream);
  m.attr("RansDecoder") = decoder;
}
